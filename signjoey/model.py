...
class SignModel(nn.Module):
    ...
    def forward(
        ...
        """
        encoder_output, encoder_hidden, attention_weights = self.encode(
        ...
        return decoder_outputs, gloss_probabilities, attention_weights
    ...
    def encode(
        ...
        encoder_output, encoder_hidden, attention_weights = self.encoder(
            embed_src=self.sgn_embed(x=sgn, mask=sgn_mask),
            # embed_src=sgn,
            src_length=sgn_length,
            mask=sgn_mask,
        )
        # print("Source attention weights", attention_weights.shape)

        return encoder_output, encoder_hidden, attention_weights
    ...
    def get_loss_for_batch(
        ...
        # Do a forward pass
        decoder_outputs, gloss_probabilities, attention_weights = self.forward(
            sgn=batch.sgn,
